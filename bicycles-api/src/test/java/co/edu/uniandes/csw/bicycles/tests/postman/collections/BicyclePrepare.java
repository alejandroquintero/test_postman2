 ...
 */
package co.edu.uniandes.csw.bicycles.tests.postman.collections;
 ...
 * @author Asistente
 */
public class BicyclePrepare  {
    
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionBicycle.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,BicycleDetailDTO bicycles,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(bicycles);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(BicycleDetailDTO bicycles) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:bicycles.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(bicycles, null).toString().concat("\",\""))); 
           }
