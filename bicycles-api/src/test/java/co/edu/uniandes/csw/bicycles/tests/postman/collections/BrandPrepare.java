 ...
 * @author Asistente
 */
public class BrandPrepare  {
    
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionBrand.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,BrandDetailDTO brands,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(brands);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(BrandDetailDTO brands) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:brands.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(brands, null).toString().concat("\",\""))); 
           }
